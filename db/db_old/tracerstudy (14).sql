...
--
-- Host: 127.0.0.1
-- Generation Time: Jun 05, 2021 at 04:22 PM
...
SELECT COUNT(responden) INTO respon FROM (
select COUNT(j.id_user) as responden from ((`tracerstudy`.`survei` `s` left join `tracerstudy`.`jawaban_pertanyaan` `j` on(`s`.`id` = `j`.`id_survei`)) left join `tracerstudy`.`user` `u` on(`j`.`id_user` = `u`.`id`)) WHERE j.id_survei = idsurvei group by `j`.`id_survei`,`j`.`id_user`) jawaban_pertanyaan;
...
(1, 'Universitas Sumatera Utara', 'Teknologi Informasi', '#1c7382', '#44e4bc', 'itusu@gmail.com', '086789090000', '', 'Tracer Study Alumni Teknologi Informasi USU');
...
(5, 'SURVEI TRACER STUDY', 'Survei ini dibuat berdasarkan borang tracer study dari dikti', '2021-06-11', '2021-06-16', '2021-06-03'),
(6, 'SURVEI TRACER STUDY', 'Pada saat lulus, pada tingkat mana kompetensi di bawah ini anda kuasai? ', '2021-06-03', '2021-06-12', '2021-06-03'),
(7, 'SURVEI TRACER STUDY', 'Pada saat lulus, bagaimana kontribusi perguruan tinggi dalam hal kompetensi di bawah ini?', '2021-06-03', '2021-06-26', '2021-06-03');
...
DELETE FROM survei_pertanyaan WHERE id_survei = old.id;
DELETE FROM survei_jawaban WHERE id_survey = old.id;
DELETE FROM jawaban_pertanyaan WHERE id_survei = old.id;
END
$$
DELIMITER ;
...
(193, 5, 74, 'sdsds'),
(194, 8, 75, 'Melalui iklan di koran/majalah, bros');
...
CREATE ALGORITHM=UNDEFINED DEFINER=`root`@`localhost` SQL SECURITY DEFINER VIEW `jawaban_v`  AS SELECT `sj`.`id_survey` AS `id_survey`, `p`.`pertanyaan` AS `pertanyaan`, `sj`.`jawaban` AS `jawaban`, `sj`.`id_pertanyaan` AS `id_pertanyaan`, `sj`.`id` AS `id`, count(`j`.`jawaban`) AS `jlh` FROM ((`survei_jawaban` `sj` left join `jawaban_pertanyaan` `j` on(`j`.`jawaban` = `sj`.`id`)) left join `survei_pertanyaan` `p` on(`sj`.`id_pertanyaan` = `p`.`id`)) GROUP BY `sj`.`id` ORDER BY `sj`.`id` DESC ;
...
--
ALTER TABLE `survei_jawaban`
  MODIFY `id` int(3) NOT NULL AUTO_INCREMENT, AUTO_INCREMENT=196;
