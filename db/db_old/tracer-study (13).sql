...
select COUNT(j.id_user) as responden from ((`tracer-study`.`survei` `s` left join `tracer-study`.`jawaban_pertanyaan` `j` on(`s`.`id` = `j`.`id_survei`)) left join `tracer-study`.`user` `u` on(`j`.`id_user` = `u`.`id`)) WHERE j.id_survei = idsurvei group by `j`.`id_survei`,`j`.`id_user`) jawaban_pertanyaan;
...
(9, 1, 'Software Development', 'Jakarta, Indonesia', 'Goto Company', 'goto@gmail.com', 'Tertera', 'Ayo daftarkan dirimu menjadi bagian dari keluarga Goto Company', '2021-06-01', '2021-07-09', 'unpublish');
